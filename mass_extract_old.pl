...
=item * UnrarExtractor - RAR extraction operations (bundled)

=item * ExtractionLogger - CSV logging functionality (bundled)

=item * MassExtractGUI - Tk GUI interface (bundled, required for GUI mode)
...
my $gui_obj = $gui ? MassExtractGUI->new() : undef;
